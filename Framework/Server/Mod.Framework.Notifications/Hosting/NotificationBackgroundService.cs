...
namespace Mod.Framework.Notifications.Hosting
{
    public abstract class NotificationBackgroundService : BackgroundService, INotificationBackgroundService
    {
        ...
        private void GenerateNotifications()
        {
            ...
            var templates = this.TemplateRepository.GetAll().Where(x => x.IsEnabled && x.Frequency != (int)NotificationFrequency.REAL_TIME && (x.NextRunTime ?? DateTime.Now.AddMilliseconds(-500)) <= DateTime.Now);

            foreach (NotificationTemplate template in templates)
            {
                // Check for next run date, if null, set to now(ish)
                template.NextRunTime = template.NextRunTime ?? DateTime.Now.AddMilliseconds(-500);

                if (template.NextRunTime <= DateTime.Now)
                {
                    ...
                    // Save immediately in the hopes of preventing other servers from picking up the same template
